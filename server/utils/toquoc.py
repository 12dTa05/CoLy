def crawl_toquoc_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='sapo')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='data-content-body')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'toquoc', 'crawled_at': datetime.utcnow()}