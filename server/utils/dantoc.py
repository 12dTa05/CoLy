def crawl_dantoc_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='news-sapo')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='CustomContentObject PhotoObject ObjectAlignCenter')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'dantoc', 'crawled_at': datetime.utcnow()}