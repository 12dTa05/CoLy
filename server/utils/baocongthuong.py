def crawl_baocongthuong_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('div', class_='article-detail-desc f2 fw lt clearfix')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='fw clearfix')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'baocongthuong', 'crawled_at': datetime.utcnow()}