def crawl_thoibaonganhang_article(keyword, link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('div', class_='article-detail-desc f0 fw')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='__MASTERCMS_CONTENT fw f1 mb clearfix')
        if content_tag:
