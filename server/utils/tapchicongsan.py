def crawl_tapchicongsan_article(keyword, link, driver, title, pubDate):
    try:
        ...
        article_data = {
            ...
            'content': None,
            'author': None,
            'source': 'tapchicongsan',
            'summary': None,
            'crawled_at': datetime.now(timezone.utc)
        }
        
        description_tag = soup.find('div', class_='sumary')
        article_data['description'] = description_tag.find('span').text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='detailContent')
        if content_tag:
