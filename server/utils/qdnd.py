def crawl_qdnd_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('div', class_='post-summary')
        article_data['description'] = description_tag.find('p').text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='articleBody')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'qdnd', 'crawled_at': datetime.utcnow()}