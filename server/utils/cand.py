def crawl_cand_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('div', class_='box-des-detail this-one')
        article_data['description'] = description_tag.find('p').text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='detail-content-body')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'cand', 'crawled_at': datetime.utcnow()}