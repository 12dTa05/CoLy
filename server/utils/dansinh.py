def crawl_dansinh_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='italic font-noto-serif')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='content font-noto-serif')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'dansinh', 'crawled_at': datetime.utcnow()}