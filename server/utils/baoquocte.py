def crawl_baoquocte_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('div', class_='article-detail-desc fw lt f0 mb clearfix')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='fw lt clearfix')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'baoquocte', 'crawled_at': datetime.utcnow()}