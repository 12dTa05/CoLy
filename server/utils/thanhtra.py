def crawl_thanhtra_article(keyword, link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('p', class_='text-lg text-justify font-semibold mb-4')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='editor-detail')
        if content_tag:
