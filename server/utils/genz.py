def crawl_genz_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='jeg_post_subtitle')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='content-inner')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'genz', 'crawled_at': datetime.utcnow()}