def crawl_baodautu_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='sapo_detail')
        article_data['description'] = description_tag.text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='content_detail_news')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'baodautu', 'crawled_at': datetime.utcnow()}