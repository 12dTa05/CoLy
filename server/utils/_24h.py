def crawl_24h_article(keyword, link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='cate-24h-foot-arti-deta-sum ctTp tuht_show')
        article_data['description'] = description_tag.find('strong').text.strip() if description_tag else None
        
        content_tag = soup.find('article', class_='cate-24h-foot-arti-deta-info')
        if content_tag:
