def crawl_baochinhphu_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('div', class_='td-fix-index')
        article_data['description'] = description_tag.find('p').text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='tdb-block-inner td-fix-index')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'congngheviet', 'crawled_at': datetime.utcnow()}