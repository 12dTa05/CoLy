def crawl_giaoducthoidai_article(link, driver, title, pubDate):
    try:
        ...
        description_tag = soup.find('h2', class_='article__sapo cms-desc')
        article_data['description'] = description_tag.find('p').text.strip() if description_tag else None
        
        content_tag = soup.find('div', class_='article__body zce-content-body cms-body')
        if content_tag:
        ...
        return {'link': link, 'error': str(e), 'source': 'giaoducthoidai', 'crawled_at': datetime.utcnow()}